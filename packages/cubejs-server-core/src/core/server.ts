...
export class CubejsServerCore {
  ...
  protected reloadEnvVariables() {
    if (!this.options.dbType && process.env.CUBEJS_DB_TYPE) {
      this.options.dbType = <DatabaseType | undefined>process.env.CUBEJS_DB_TYPE;
      this.contextToDbType = wrapToFnIfNeeded(this.options.dbType);
    }

    if (!this.options.externalDbType && process.env.CUBEJS_EXT_DB_TYPE) {
      this.options.externalDbType = <DatabaseType|undefined>process.env.CUBEJS_EXT_DB_TYPE;
      this.contextToExternalDbType = wrapToFnIfNeeded(this.options.externalDbType);
    }
  }
